...
#'
#' @param point1 vector with two numeric values for the first pair of
#'   latitude-longitude coordinates;
#'   _e.g._, c(latitude, longitude), c(42.331177, -83.046023)  ## Detroit, Michigan
#' @param point2 vector with two numeric values for the first pair of
#'   latitude-longitude coordinates;
#'   _e.g._, c(latitude, longitude), c(42.737652, -84.483788)  ## East Lansing, Michigan
#' @param distance.units string indicating if the resulting distance should be
#'   miles or kilometers; default is `"miles"`.
#'
#'
...
LatLonDistances <- function(lat1, long1, lat2, long2, distance.units="miles") {
  ...
  dist.lat <- (lat2 - lat1)
  dist.long <- (long2 - long1)

  ## calculate the difference between the two points ----
  a <- ( sin(dist.lat/2) )^2 + cos(lat1) * cos(lat2) * ( sin(dist.long/2) )^2
...
#'   stackoverflow response by [Josh O'Brien](https://stackoverflow.com/users/980833/josh-obrien).
#'
...
#' lonlat2regionfunction(longitude, latitude, gadm.mapfile=mapfile.world)
#' }
#'
#' @author Emilio Xavier Esposito \email{emilio@@msu.edu}
#'   ([https://github.com/emilioxavier](https://github.com/emilioxavier))
